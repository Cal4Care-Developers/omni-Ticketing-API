                        ...
                        janus.attach(
                            {
                                ...
                                mediaState: function(medium, on) {
                                  //  Janus.log("Omni " + (on ? "started" : "stopped") + " receiving our " + medium);
                                },
                                webrtcState: function(on) {
                                   //alert("Omni says our WebRTC PeerConnection is " + (on ? "up" : "down") + " now");
                                    ...
                                    if(result && result["event"]) {
                                        ...
                                        if(event === 'registered') {
                                            ...
                                            if(!registered) {
                                                ...
                                                //  '<button id="addhelper" style="" class="btn btn-xs btn-info pull-right" title="Add a new line">' +
                                            ...
                                            //$('#call').removeAttr('disabled').html('Hangup')
                                                ...
                                                $('#dialpad-wrapper').attr('style', 'display: block;');
   ...
   console.log(data);
            window.parent.postMessage(data, '*');
                                            ...
                                            Janus.log("Incoming calls from " + result["username"] + "!");
											 var numbe = result["username"];
                                                numbe = numbe.replace("sip:", '');
                                                numbe = numbe.substring(0, numbe.indexOf("@") - 0);
                                                // alert(text);
											notifyMe("Call from " + numbe + "!", "Incoming Call");
...
//     let dialpad_req = new Object();
                                                ...
                                                $.ajax({
                                                    ...
                                                    success: function(response){
                                                        ...
                                                        if (response.result.status == 1) { 
                                                            ...
                                                            var outgoingC = '<div><div class="call-screen-panel call_id_1"><div class="active-call"><div class="caller-img"><img src="assets/images/user.jpg"></div><h4><b _ngcontent-vqe-c2="">Incoming Call</b></h4><h5>'+incCall+'</h5><div class="call-icon incoming-call"><span class="ringing_phone"><i class="fa fa-phone"></i></span></div></div><div class="botrow" style="display:block; margin-top:145px"><div class="dig call_accept" id="call" ><i aria-hidden="true" class="fa fa-phone"></i></div><div class="dig oncall" id="endCall" ><i class="fas fa-phone-slash"></i></div></div></div></div>';
                                                            $('#dialpad_layout').html(outgoingC); 
                                                            $('#call_history_id').val(response.result.data);
                                                            // $('#endCall').removeAttr('disabled').html('Hangup')
                                                            // .removeClass("btn-success").addClass("btn-danger")
                                                            // .unbind('click').click(doHangup);
                                        ...
                                        clearTimeout(callduration_timer);
                                            ...
                                            if(result["reason"] === "Request Terminated"){
                                                ...
                                                dialPadDetailView('call_history_detail', this.call_history_id);
...
function initCallService(){
    ...
    $.ajax({
        ...
        $('#sipExtension').val(response.data.sip_login);
        $('#sipHardwareId').val(response.data.hardware_id);
        ...
        $('#displayname').val('Omnichannel');
        
            var queueStatus = response.queueData.status;
            var queueReason = response.queueData.reason;


            if(queueStatus != '1'){
                $('#queueStatsu').html('<h1 title="'+queueReason+'" style="color:#f23333;font-size: 25px; cursor: pointer;" onclick="QueueLogin()">Q</h1>');
            }
...
function reg(sip_login,sip_authentication,sip_password,sip_url,sip_port){
    var register = {
        request: "register",
        username: sip_login
    };
...
function registerUsername() {
    selectedApproach="secret";
    if(!selectedApproach) {
        //console.log("Please select a registration approach from the dropdown menu");
        return;
    }
    // Try a registration
    ...
    if(selectedApproach === "guest") {
        ...
        if(!username === "" || username.indexOf("sip:") != 0 || username.indexOf("@") < 0) {
           // console.log("Please insert a valid SIP address (e.g., sip:goofy@example.com): this doesn't need to exist for guests, but is required");
            $('#server').removeAttr('disabled');
            $('#username').removeAttr('disabled');
            $('#authuser').removeAttr('disabled');
            $('#displayname').removeAttr('disabled');
    ...
    var username = $('#username').val();
    if(username === "" || username.indexOf("sip:") != 0 || username.indexOf("@") < 0) {
        //console.log('Please insert a valid SIP identity address');
        $('#server').removeAttr('disabled');
        $('#username').removeAttr('disabled');
        $('#authuser').removeAttr('disabled');
        $('#displayname').removeAttr('disabled');
    ...
    janus.attach(
        {
            ...
            mediaState: function(medium, on) {
               // Janus.log("[Helper #" + helperId + "] Omni " + (on ? "started" : "stopped") + " receiving our " + medium);
            },
            webrtcState: function(on) {
               // Janus.log("[Helper #" + helperId + "] Omni says our WebRTC PeerConnection is " + (on ? "up" : "down") + " now");
            ...
            onmessage: function(msg, jsep) {
                ...
                if(result && result["event"]) {
                    ...
                    if(event === 'registration_failed') {
                        ...
                        // bootbox.alert(result["code"] + " " + result["reason"]);
                        console.log(result["code"] + " " + result["reason"]);// Get rid of the helper
                        removeHelper(helperId);
                        return;
                    }
                    if(event === 'registered') {
                                        ...
                                        sipcallAction(
                                            {
                                                ...
                                                error: function(error) {
                                                    ...
                                                    // bootbox.alert("WebRTC error... " + error.message);
                                                    console.log("WebRTC error... " + error.message);// Don't keep the caller waiting any longer, but use a 480 instead of the default 486 to clarify the cause
                                                    var body = { request: "decline", code: 480 };
...
function registerStatus2(){
   var dial_status = this.getStatus();
    ...
    if(dial_status == 'REGISTERED'){
        $('#dialstatus').attr('style', 'background-color: #33f247;');
        $('#call-text').html('Ready For Calls');
    } else if(dial_status == 'ESTABLISHED'){
        $('#call-text').html('Established');
        $('#dialstatus').attr('style', 'background-color: #f2b733;');
    } else {
 		...
 		$('#dialpad_layout').show();
            ...
            if (response.result.status == 1) {
                ...
                //  mainPhone = "'"+mainPhone+"'";
                    ...
                    var number_dialer = "'number_dailer'";
                    var outgoingC = '<div class="contact-panel"><input type="hidden" id="dialpad_number2"><div class="call-screen-panel call_id_2"><div class="backArrow" onclick="dialPadview('+number_dialer+')" data-toggle="tooltip" title="Back"><i aria-hidden="true" class="fa fa-arrow-left"></i></div><div class="active-call"><div class="caller-img"><img src="assets/images/user.jpg"></div><h4><b>Call Details</b></h4><h5>'+response.result.data.call_data+'</h5><div class="call-duration">'+response.result.data.call_start_dt+'</div></div><div class="botrow" style="display:block"><div class="dig" onclick="dialPadview('+number_dialer+')"><i aria-hidden="true" class="fa fa-th"></i><span class="btn-icon-name">Keypad</span></div><div class="dig" id="call" onclick="doCall2()"><i aria-hidden="true" class="fa fa-phone"></i></div></div></div></div>';
                    $('#dialpad_layout').html(outgoingC); 
...
function makecallTransfer(){
    
    var sip_urld = $('#sip_urld').val();
    var username = $('#makeCallForwordNumber').val();
    $('#outcall_number').val(username);
    username = "sip:"+username+"@"+sip_urld;
...
function viewtransfercontact() {
	 var Murl = window.location.href; //for current url
	 var login = /login=([^&]+)/.exec(Murl)[1]; // Value is in [1] ('384' in our case)
		 let api_req = new Object();
		 let dialpad_req = new Object();
		 dialpad_req.login = login;
		 dialpad_req.action = "user_list_login";
		 ...
		 $.ajax({
			 ...
			 success: function(response){
				 ...
				 if (response.result.status == true) {					
						...
						//	 alert("user_list_login");
							 var  data  = response.result.data;
							 contentStr = "";
							 var d = '';
					 var dummy= '';
							 data.forEach(function(user_list){
 ...
 function transfercontact(number){
	var hash = "'#'";
	var star = "'*'";
	...
	var number_dialerC = '<div class="tab-content"  id="trans_keypad"> <div class="tab-pane fade show active"  role="tabpanel" aria-labelledby="trans_keypad-tab"> <div class="search-field"> <input class="form-control" id="makeCallForwordNumber" value="'+num+'"  type="text"> </div> <div class="row noMargin"> <div class="digit" id="one" onclick="TkeyPad(1)" (keypress)="TkeyPad(1)">1</div> <div class="digit" id="two" onclick="TkeyPad(2)">2 <div class="sub">ABC</div></div> <div class="digit" id="three" onclick="TkeyPad(3)">3 <div class="sub">DEF</div></div> </div> <div class="row noMargin"> <div class="digit" id="four" onclick="TkeyPad(4)">4 <div class="sub">GHI</div></div> <div class="digit" id="five" onclick="TkeyPad(5)">5 <div class="sub">JKL</div></div> <div class="digit" id="six" onclick="TkeyPad(6)">6 <div class="sub">MNO</div></div> </div> <div class="row noMargin"> <div class="digit" id="seven" onclick="TkeyPad(7)">7 <div class="sub">PQRS</div></div> <div class="digit" id="eight" onclick="TkeyPad(8)">8 <div class="sub">TUV</div></div> <div class="digit" id="nine" onclick="TkeyPad(9)">9 <div class="sub">WXYZ</div></div> </div> <div class="row noMargin"> <div class="digit" id="star" onclick="TkeyPad('+star+')">*</div> <div class="digit" id="zero" onclick="TkeyPad(0)">0</div> <div class="digit" id="hash" onclick="TkeyPad('+hash+')"># </div> </div> </div> </div> <div class="row noMargin" id="call-action-btn-group"> <ul class="nav nav-tabs" id="myTab" role="tablist"> <li class="nav-item"> <a class="nav-link" id="trans_contact-tab" data-toggle="tab" onclick="viewtransfercontact()" role="tab" aria-controls="trans_contact" aria-selected="false">  <div class="dig list_details"><i class="fa fa-user-circle" aria-hidden="true"></i></div></a> </li> <li class="nav-item"> <a class="nav-link" id="trans_keypad-tab" data-toggle="tab" href="#trans_keypad" role="tab" aria-controls="trans_keypad" aria-selected="true"><div  id="transfer" onclick="makecallTransfer()" ><span class="call-btn-icon"><img  src="assets/images/call-forward.png" height="32"></span></div> </a> </li> <li class="nav-item" onclick="dialPadbackSpaceT()"> <a class="nav-link" id="trans_keypad-tab" data-toggle="tab" href="#trans_keypad" role="tab" aria-controls="trans_keypad" aria-selected="false"><div class="digit has-img" id="" ><img src="assets/images/dialpad-clear-icon.png" height="20"></div></a> </li> </ul> </div>';
	$('#trans_contact').html(number_dialerC);
} 
    ...
    //alert(view_type);
     ...
     if (view_type != "number_dailer") {
		...
		$('#loaders').show();
         ...
         let api_req = new Object();
         let dialpad_req = new Object();
      //   dialpad_req.user_id = '64';
         dialpad_req.login = login;
         dialpad_req.action = view_type;
         if (view_type == "recent_list_widget") {
       ...
       //  api_req.login = login;
         ...
         $.ajax({
             ...
             success: function(response){
                 ...
                 if (response.result.status == true) {
                     if (view_type == "recent_list_widget") {
                         var  data = response.result.data.list_data;
                         contentStr = "";
                         var d ='';var call_id = '';
                         data.forEach(function(recent_call){
                             d = "'"+recent_call.phone+"'";      
							 call_id = "'"+recent_call.callid+"'";

                             contentStr += '<div class="contact-list-item" phone-attr="phone" onclick="webMakeCall('+d+')"><h5>'+ recent_call.call_data +'</h5><div class="call-time"><span class="callType"><img src="assets/images/outgoing-call.svg"></span>'+  recent_call.call_start_dt +'</div></div>';
                         });
                         var number_dialer = "'number_dailer'";
                         var user_lists = "'user_list_login'";
                         var recent_lists = "'recent_list_widget'";
						// <div class="search-field"><input id="dialpad_number_search" onkeyup="searchDialerrecents()" class="form-control" type="text" name="searchfield" placeholder="Search Recent Calls"></div>
                         var rCal = '<div class="contact-panel">     <div class="tab-content"><div id="recentCalls">'+contentStr+'</div><div class="botrow"><div class="dig list_details" onclick="dialPadview('+user_lists+')"><i class="fa fa-user-circle" aria-hidden="true"></i><span class="btn-icon-name">Contacts</span></div><div class="dig list_details" onclick="dialPadview('+number_dialer+')"><i class="fa fa-th" aria-hidden="true"></i><span class="btn-icon-name">Keypad</span></div><div class="dig list_details" onclick="dialPadview('+recent_lists+')"><i style="font-size: 22px;" class="fas fa-clock"></i><span class="btn-icon-name">Recents</span></div></div></div></div>';
                         $('#dialpad_layout').html(rCal); 
 
                     } else if (view_type == "user_list_login") {
                         var  data  = response.result.data;
                         contentStr = "";
                         var d = '';
                         data.forEach(function(user_list){
                             d = "'"+user_list.sip_login+"'";
                             contentStr += '<div class="contact-list-item" phone-attr="phone" onclick="webMakeCall('+d+')"><div class="contact-number-details"><span class="user-img"><img src="assets/images/user.jpg"></span><h5>'+ user_list.user_name +' ('+ user_list.agent_name +')</h5></div></div>';
                         });
 
                         var number_dialer = "'number_dailer'";
                         var user_lists = "'user_list_login'";
                         var recent_lists = "'recent_list_widget'";
                         var rCal = '<div class="contact-panel"><div class="search-field"><input id="dialpad_number_contscts" onkeyup="searchDialerContacts()" class="form-control" type="text" name="searchfield" placeholder="Search User Contacts"></div><div class="tab-content-data"><div class="tab-pane" id="userList">'+contentStr+'</div><div class="botrow"><div class="dig list_details" onclick="dialPadview('+user_lists+')"><i class="fa fa-user-circle" aria-hidden="true"></i><span class="btn-icon-name">Contacts</span></div><div class="dig list_details" onclick="dialPadview('+number_dialer+')"><i class="fa fa-th" aria-hidden="true"></i><span class="btn-icon-name">Keypad</span></div><div class="dig list_details" onclick="dialPadview('+recent_lists+')"><i style="font-size: 22px;" class="fas fa-clock"></i><span class="btn-icon-name">Recents</span></div></div></div></div>';
                         $('#dialpad_layout').html(rCal); 
                     }
...
//console.log(result_message);
 ...
 var login = /login=([^&]+)/.exec(Murl)[1]; // Value is in [1] ('384' in our case)
    ...
    //console.log(result_message[0].login);
 if(result_message[0].login == login){
     // alert(result_message[0].type);
    if(result_message[0].type == 'clickToCall'){
        //alert('fsd');
        $('#dialpad_number').val(result_message[0].number)
            $('#dialpad-wrapper').attr('style', 'display: block;');
                                             var data = '{"operation_type":"openMainChat","status_style":"position: fixed;right: -15px;bottom: 0px;border:none;height:100%;width:55%;z-index:99"}';
   console.log(data);
            window.parent.postMessage(data, '*');
...
function myQueues(){
	...
	var has_mrvoip = $('#has_mrvoip').val(); 
	
	if(has_mrvoip == '0'){
		$('#dialpad_layout').show();    
		$('#loaders').hide();
		var number_dialer = "'number_dailer'";
		var queues = '<div class="contact-panel"><form class="queuedetail"><h3 class="title-section"><span><i class="fa fa-arrow-left" onclick="dialPadview('+number_dialer+')"></i></span>Queue Details</h3><div class="flex py-2"><div class="relative w-full border-gray-400"></div></div><div class="row has-row-space"><div class="col-md-12"><div class="addAgent"><h5>Sorry!!</h5><h5>No MR.VoIP Found, Please Contact Admin.</h5></div></div></div></form></div>';
		$('#dialpad_layout').html(queues); 
		return false;
  ...
  var query = { operation: "queue", moduleType: "queue", api_type: "web", element_data: { action: "user_queue_widget",login:login } };
  ...
  $.ajax({
      ...
      success: function (response) {
		$('#dialpad_layout').show();    
		$('#loaders').hide();
        var aux = response.result.data.auxcodes;
        var queue = response.result.data.queues;

        console.log(aux);
        console.log(queue);

        options = [];
        options.push( '<option value="0">Select Aux Code</option>');

        for (let index = 0; index < aux.length; index++) {
         var data = aux[index];
         options.push( '<option value=' + data.auxcode_name + '>' + data.auxcode_name + '</option>');
         }
    ...
    for (let index = 0; index < queue.length; index++) {
     ...
     myAllQueues.push('<tr><td style="width: 20px;"><div class="pretty p-default"> <input type="checkbox" class="getallmyqueues" value="'+data.queue_number+'"></div></td><td>'+data.queue_name+'</td></tr>')
     }

     var number_dialer = "'number_dailer'";
    //var myAllQueues = '<tr><td style="width: 20px;"><div class="pretty p-default"> <input type="checkbox" class="getallmyqueues" value="802"></div></td><td>CRC Work 123</td></tr><tr><td style="width: 20px;"><div class="pretty p-default"> <input type="checkbox" class="getallmyqueues" value="803"></div></td><td>Queue2</td></tr>';
    var queues = '<div class="contact-panel"><form class="queuedetail"><h3 class="title-section"><span><i class="fa fa-arrow-left" onclick="dialPadview('+number_dialer+')"></i></span>Queue Details</h3><div class="flex py-2"><h4>Aux Code</h4><div class="relative w-full border-gray-400">'+selectBoxAux+'</div></div><div class="row has-row-space"><div class="col-md-12"><div class="addAgent"><div class="table-responsive"><table class="table table-bordered table-striped table-md"><thead><tr><th style="width: 20px;"><div class="pretty p-default"> <input type="checkbox" id="getallmyqueue" onclick="selectAllQueues()"></div></th><th>Queue Name</th></tr></thead><tbody>'+myAllQueues+'</tbody></table></div></div></div></div><div class="botrow" style="display:block"><div class="buttons"> <input type="button" value="Cancel" onclick="dialPadview('+number_dialer+')" class="btn btn-outline-secondary"> <input type="button" onclick="qLogout()" value="Logout" class="btn btn-primary update_agent"></div></div></form></div>';
...
function qLogout (reason,queues){

    var queues = $('.getallmyqueues:checked').map(function(){
        return this.value;
    }).get();

        var reason = $('#auxCode').val();

        var extension = atob($('#sipExtension').val());
        var has_hard_id = atob($('#sipHardwareId').val());
        var socket_message  =  '[{"cust_id":"'+has_hard_id+'","data":[{"Name":"queuestatus","extension":"'+extension+'","status":"0","reason":"'+reason+'","queues":"'+queues+'"}]}]';
        dialPadview('number_dailer');
        websocket.send(socket_message); 
        $('#queueStatsu').html('<h1 title="'+reason+'" style="color: #f23333;font-size: 25px; cursor: pointer;" onclick="QueueLogin()">Q</h1>');
   }
...
function QueueLogin (){
  ...
  var query = { operation: "queue", moduleType: "queue", api_type: "web", element_data: { action: "in_login_logout",login:login,reason: "-","status":"1" } };
  ...
  $.ajax({
      ...
      success: function (response) {
        ...
        var extension = atob($('#sipExtension').val());
        var has_hard_id = atob($('#sipHardwareId').val());
        var socket_message  =  '[{"cust_id":"'+has_hard_id+'","data":[{"Name":"queuestatus","extension":"'+extension+'","status":"1","reason":"0","queues":""}]}]';
        websocket.send(socket_message); 
        $('#queueStatsu').html('<h1 style="color: #33f247;font-size: 25px; cursor: pointer;" onclick="myQueues()">Q</h1>');
