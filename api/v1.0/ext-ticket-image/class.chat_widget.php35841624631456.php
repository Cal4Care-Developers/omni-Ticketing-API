...
class chat_widget extends restApi{
      ...
      if(isset($_FILES["chat_image"]))
        {
          ...
          if (! in_array($chat_file_extension, $allowed_chat_image_extension)) {
			  ...
			  $chat_image_upload_path = "https://".$_SERVER['SERVER_NAME']."/api/v1.0/chat_image/". basename( $_FILES["chat_image"]["name"]);  
  ...
  public function chat_timezone($chat_data){
    ...
    $day = date('l', strtotime($datevalue)); 		
     if($day=='Monday'){
      ...
      $str_time = strtotime($close_time);	  
	  if($client_str_time < $str_time){
        $available_status = 1;
      }else{
        $available_status = 0;
      }
  ...
  $dept_qry = "SELECT * FROM departments WHERE admin_id='$user_id' AND status=1 AND delete_status=0";
    ...
    $result['enable_dept'] = $enable_dept;
		...
		<h6>Live Chat By "'.$customer_name.'"</h6>';
		  foreach($result as $data){
			 ...
			 if($msg_user_id == $user_id){
			     $results .= '<div class="agent-msg msg-row"><img class="agent-avatar" src="'.$profile_image.'"><p>'.$chat_msg.'</p><span class="agent_name">'.$user_name.'</span></div>';    
		     }else{
			     $results .= '<div class="customer-msg msg-row"><img src="images/user-theme.svg"><p>'.$chat_msg.'</p><span>You</span></div>';
		     }
		 } 
		 return $results;
      }
   ...
   public function dept_agent_list($chat_data){ 
     ...
     $get_qry = "SELECT user_id,agent_name,profile_image FROM user WHERE admin_id='$admin_id' AND has_chat=1 AND user_status=1 AND delete_status=0";    
