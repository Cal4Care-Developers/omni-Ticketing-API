...
if($action == "admin_global_settings"){    
   // $timezone_id = $json_element_data['timezone_id']; 
    ...
    $agent_data = array("timezone_id"=>$timezone_id,"ext_int_status"=>$ext_int_status,"show_caller_id"=>$show_caller_id,"logo_image"=>$logo_image,"small_logo_image"=>$small_logo_image,"img_user_id"=>$img_user_id);
...
if($action == "dialer_settings"){
	$ext_number = base64_decode($ext_number);
    $data = array("ext_number"=>$ext_number,"company_name"=>$company_name);
    $result_data["result"]["data"] = $user->dialer_settings($data);
}
