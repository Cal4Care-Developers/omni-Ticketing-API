...
public function updateDepartment($data){
	extract($data);
            ...
            if($update_data != 0){
				$qry=$this->db_query("DELETE From dep_drag", array());
                $ex=explode(',',$department_users);
				for($i=0; $i<count($ex); $i++){
					$us=$ex[$i];
...
$user_qry = "SELECT timezone_id FROM user WHERE user_id='$user_id'";
  ...
  public function ticketReport($data){
		...
		if($user_type == 2){
			...
			if($ticket_status == 'All' && $ticket_department == 'All'){
					...
					$detail_qry = $qry."  ORDER BY ticket_no DESC LIMIT $limit offset $offset";
				 	...
				 	$qry = "SELECT * FROM external_tickets WHERE admin_id = $user_id and ticket_department LIKE '%$ticket_department%' ";
					$detail_qry = $qry."ORDER BY ticket_no DESC LIMIT $limit offset $offset";
			} else if($ticket_status != 'All' && $ticket_department == 'All'){
					$qry = "SELECT * FROM external_tickets WHERE admin_id = $user_id and ticket_status = $ticket_status ";
				 	$detail_qry = $qry."ORDER BY ticket_no DESC LIMIT $limit offset $offset";
			}  else {
				$qry = "SELECT * FROM external_tickets WHERE admin_id = $user_id and ticket_status = $ticket_status and ticket_department LIKE '%$ticket_department%'";
				 $detail_qry = $qry." ORDER BY ticket_no DESC LIMIT $limit offset $offset";
			}
			...
			if($ticket_status == 'All' && $ticket_department == 'All'){
				$qry = "SELECT a.*, b.* FROM external_tickets a JOIN external_tickets_data b ON a.ticket_no = b.ticket_id WHERE FIND_IN_SET('$user_id',a.ticket_assigned_to ) OR a.ticket_created_by = $user_id ";
				 $detail_qry = $qry." Group by a.ticket_no ORDER BY a.ticket_no DESC LIMIT $limit offset $offset";
				 	...
				 	$qry = "SELECT a.*, b.* FROM external_tickets a JOIN external_tickets_data b ON a.ticket_no = b.ticket_id WHERE FIND_IN_SET('$user_id',a.ticket_assigned_to ) and a.ticket_department LIKE '%$ticket_department%' OR a.ticket_created_by = $user_id ";
				 $detail_qry = $qry." Group by a.ticket_no ORDER BY a.ticket_no DESC LIMIT $limit offset $offset";
			} else if($ticket_status != 'All' && $ticket_department == 'All'){
					$qry = "SELECT a.*, b.* FROM external_tickets a JOIN external_tickets_data b ON a.ticket_no = b.ticket_id WHERE FIND_IN_SET('$user_id',a.ticket_assigned_to ) and a.ticket_status = '$ticket_status' OR a.ticket_created_by = $user_id ";
				 $detail_qry = $qry." Group by a.ticket_no ORDER BY a.ticket_no DESC LIMIT $limit offset $offset";
			}  else {
				$qry = "SELECT a.*, b.* FROM external_tickets a JOIN external_tickets_data b ON a.ticket_no = b.ticket_id WHERE FIND_IN_SET('$user_id',a.ticket_assigned_to ) and a.ticket_status = '$ticket_status' and a.ticket_department LIKE '%$ticket_department%' OR a.ticket_created_by = $user_id ";
				 $detail_qry = $qry." Group by a.ticket_no ORDER BY a.ticket_no DESC LIMIT $limit offset $offset";
			}
		}		
	...
	public function oncloseTocket($data){
		...
		$repM = $this->fetchOne($e, array());
				...
				foreach($messages as $m){
					$mess[] = '<div style="border: 1px solid #d1d1d1; border-radius: 8px; padding: 12px; margin-bottom: 25px;">'.$m.'</div>';
				}
				...
				$uss = array("ticket_to"=>"$ticket_to","ticket_cc"=>"$ticket_cc","from"=>"$from","message"=>$messagetoSend,"subject"=>$subject, "ticket_id"=>$ticket_no,"message_id"=>$message_id);				
	...
	public function onchangeDepartment($data){
        ...
        $qry = "UPDATE external_tickets SET ticket_department='$department_id',ticket_assigned_to='$get_dep' where ticket_no='$ticket_id'";
...
public function get_string_between($string, $start, $end){
	...
	public function add_notAssigned_tickets($data){ 
       ...
       // $qry = "SELECT * FROM admin_details WHERE support_email IN $to"; 
        ...
        if($qry_value > 0){
			...
			/*if($to2 == 'nzassaabloycc@pipe.mconnectapps.com'){
				$dept = '3';
			} else if($to2 == 'ausassaabloycc@pipe.mconnectapps.com'){
				$dept = '8';
			} else {
				$dept = '';
			} */
			...
			if(substr( $subject, 0, 3 ) === "Re:" || substr( $subject, 0, 3 ) === "RE:"){
				...
				$qryss = "UPDATE `external_tickets` SET status = '1' WHERE ticket_no = '$ticket_no'";             
				...
				$qry_result = $this->db_insert("INSERT INTO external_tickets_data(ticket_id,ticket_message,ticket_subject,replied_from,replied_to,replied_cc,ticket_media,ticket_reply_id) VALUES ( '$ticket_no','$message','$subject','$from','$to','$cc','$attachments','$ticket_reply_id')", array());
				 $dt = date('Y-m-d H:i:s');
				...
				foreach($dept_users as $user){
					$this->db_query("Insert into mc_event (user_id,admin_id,mc_event_key,mc_event_data,mc_event_type,event_status, created_dt) values('$user','$admin_id','$ticket_no','Reply From $from  ($subject)','11','7','$dt')", array());
			 ...
			 $ticket_no = $this->db_insert("INSERT INTO external_tickets(ticket_from,admin_id,ticket_status,priority,ticket_subject,ticket_to,ticket_assigned_to,next_assign_for,ticket_department,status) VALUES ( '$from','$admin_id','1','2','$subject','$to2','$assugnTo','$next_array_val','$dept','1')", array());
				...
				$qry_result = $this->db_insert("INSERT INTO external_tickets_data(ticket_id,ticket_message,ticket_subject,replied_from,replied_to,replied_cc,ticket_media,ticket_reply_id) VALUES ( '$ticket_no','$message','$subject','$from','$to','$cc','$attachments','$ticket_reply_id')", array());
				//$tt = "INSERT INTO external_tickets_data(ticket_id,ticket_message,ticket_subject,replied_from,replied_to,replied_cc.ticket_media,ticket_reply_id) VALUES ( '$ticket_no','$message','$subject','$from','$to','$cc','$attachments','$ticket_reply_id'";
				//print_r($tt); exit;
			...
			foreach($assugnTo as $user){
					$this->db_query("Insert into mc_event (user_id,admin_id,mc_event_key,mc_event_data,mc_event_type,event_status, created_dt) values('$user','$admin_id','$ticket_no','Ticket From $from  ($subject)','11','7','$dt')", array());
			...
			$replied_from = strstr($from, '<');
				...
				$uss = array("ticket_to"=>"$replied_from","ticket_cc"=>"$ccs","from"=>"$to2","message"=>$messagetoSend,"subject"=>$subject, "ticket_id"=>$ticket_no);				
				$autoRespns = $this->autoResponseEmail($uss);
				print_r($autoRespns); exit;	
			}
        }
	...
	//echo $detail_qry;exit;
		...
		// echo $assignedto_qry;exit;
          ...
          $ticket_options = array('ticket_no' => $ticket_no,'ticket_media'=>$ticket_media, 'ticket_created_by' => $ticket_from, 'ticket_assigned_to' => $ticket_assigned_to,'ticket_assigned_to_id'=>$ticket_assigned_to_id,'department' => $department,'depart_id'=>$ticket_department, 'subject'=> $ticket_subject, 'ticket_status' => $ticketstatus,'ticket_status_id'=>$ticket_status,'ticket_message'=>$ticket_message,'ticket_notes'=>$ticket_notes_by,'priority' => $priority_value, 'first_letter' => strtoupper($ticket_from[0]), 'ticket_created_at' => $created_time,'ticket_from'=>$ticket_from,'ticket_to'=>$ticket_to,'replied_from'=>$replied_from,'ticket_message_id'=>$ticket_message_id,'replied_by'=>$replied_by, 'own_mail' =>$from, 'own_img' =>$own_img, 'rep_img' =>$rep_img, 'rep_name' =>$rep_name, 'user_name'=>$user_name,'mail_cc'=>$ccMails, 'first_letter_r' => strtoupper($replied_from[0]));
          $ticket_options_array[] = $ticket_options;
        }
		...
		$side_menu = array('first_res_time' => $first_res_time, "closed_at"=>$closed_at);
		$total_count = $this->dataRowCount($qry,array());
	    $total = array('total' => $total_count);
		...
		if(!$mail->send()) {				
			print_r($mail->ErrorInfo);exit ;
				$res = "Mailer Error: " . $mail->ErrorInfo;
				 ...
				 $qry_result = $this->db_insert("INSERT INTO external_tickets_data(ticket_id,ticket_message,ticket_subject,replied_from,replied_to,replied_cc,ticket_media,repliesd_by,user_id) VALUES ( '$ticket_id','$message','$subject','$from','$to','$mail_cc','$files_arr','Agent','$user_id')", array()); 
		$result = $qry_result == 1 ? 'mailed' : 'Error';
				  $dt = date('Y-m-d H:i:s');
...
public function createExternalTicket($data){
		...
		$qry = "SELECT sig_content FROM email_signatures WHERE is_default='1' and admin_id='$dmin_id' and user_id='$user_id' ";
			$mailSignature = $this->fetchOne($qry,array());
			if($mailSignature){
				$mailSignature = str_replace('</div>', '</div></body></html>', $mailSignature);
...
public function getTicketThread($ticket_id){
    ...
    if($replied_by!='Agent') {
         $ticket_message = '<h1 style="font-size: 20px; text-align: right; background: #00a65a; color: #fff; padding: 10px;margin-top: 0; border-radius: 8px 8px 0 0;"> '.$replied_from.'<span style="font-weight: 600;font-size: 15px;border-radius: 100%;background: #3f81368f;width: 30px;height: 30px;border: 1px solid;padding: 8px;margin: 5px;">'.$mailer.'</span></h1>'.$ticket_messages;
	} else {
		
		if($rep_img ==''){ $rep_img='https://assaabloycc.mconnectapps.com/assets/images/user.jpg';}
		 $ticket_message = '<h1 style="font-size: 20px; background: #0b89a8; color: #fff; padding: 10px; margin-top: 0; border-radius: 8px 8px 0 0;"><figure style="overflow: hidden;padding: 0;margin: 0px 10px 0px 0px;float: left;"><img width="25" height="25" src="'.$rep_img.'" style="width: 25px;border-radius: 50%;"></figure>  '.$rep_name.'</h1>'.$ticket_messages;
...
//echo $token;exit;
    ...
    $notification = array('title' =>$title, 'text' => $text, 'notification_for'=>'email_ticketing', 'click_action'=>$click_url, 'unique_id'=>$ticket_id, 'sound' =>'default', 'badge' => '1','icon' => '');
    ...
    $result =  $this->fetchData($qry, array());
	 ...
	 if($result > 0){
		 $id = $result['id'];
	 	 ...
	 	 $qry = "UPDATE department_emails SET departments='$departments' where id='$id'";
		 $update_data = $this->db_query($qry, $params);
	 } else {
	 	$qry = "INSERT INTO department_emails(departments,emailAddr) VALUES ( '$departments','$email')";       
        $insert_data = $this->db_query($qry, $params);
	 }
