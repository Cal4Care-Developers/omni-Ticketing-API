...
//echo "123";exit;
  ...
  public function fax_upload($fax_data){
      ...
      if(isset($_FILES["document_file"]))
		...
		//echo  "INSERT INTO fax_details(admin_id,doc_id,name,document,type,description) VALUES ('$user_id', '$doc_id', '$name', '$main_document_upload_path', '$type', '$description')";exit;
...
public function get_fax_stat(){
	...
	public function get_admin_didnumbers($fax_data){
        extract($fax_data);//print_r($fax_data);
        $get_qry = "SELECT * FROM user WHERE user_id='$user_id' AND fax_user_id='$fax_user_id'";
        $results = $this->fetchData($get_qry,array());//print_r($results);
        if($results > 0){         
		   $user_name = $results['email_id'];
           $password = $results['password'];
			//exit;
          $token = $this->fax_curl_admin($user_name,$password);
			//echo $token;exit;
          $res = $this->get_admin_did_number_curl($token);
			//echo $res;exit;
          $cnt = count($res);
          if($cnt > 0){
            for($i=0;$i<$cnt;$i++){
