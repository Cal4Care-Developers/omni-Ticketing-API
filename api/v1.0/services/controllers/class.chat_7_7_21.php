...
function addsinglepbxsettings($sip_port,$sip_url,$admin_id){    
  ...
  /*function mcEvents($user_id){    
	...
	$has_telegram = $admin_res['has_telegram'];
		...
		$vals=$has_chat1.','.$has_whatsapp1.','.$has_telegram1.','.$has_fb1;
...
// Web Chat
        ...
        function getcustomersChatWC($user_id,$queue_id,$search_text,$status){
...
<td style="padding: 0px 30px 40px 30px;background-color:#ffffff;color: #111149; font-family: Post Grotesk, Avenir, Century Gothic, Helvetica, sans-serif; min-width: auto !important; font-size: 17px; line-height: 26px; text-align: left; font-weight: normal;"><strong style="color:#111149; min-width: 150px; width: 150px; float: left; display: inline-block;  text-decoration:none;">Reason : </strong> '.$reason.'</td> 
</tr>
</tbody> 
</table> 
</div> </td> 
...
<!--[if mso | IE]>
   ...
   <![endif]--> 
     ...
     $mail->Port = '2525';                    // set the SMTP port for the GMAIL server
      $mail->Username = 'omni_erp'; // SMTP account username
                    $mail->Password = 'MGMyd3p2end0YnNi';  
...
if($admin_id=='1'){	
		$chat_detail_qry = "select chat.chat_id, chat.chat_user, chat.chat_type, chat.chat_status, chat.rating_value, (SELECT agent_name from user where user_id = (select MAX(msg_user_id) from chat_msg WHERE chat_id = chat.chat_id and msg_user_type=2)) as chat_agent,customer.customer_name,customer.city,customer.country,customer.created_ip, TIME_FORMAT(chat.created_dt, '%H:%i') as chat_time, date_format(chat.created_dt, '%d/%m/%Y') as chat_dt, departments.department_name from chat left join customer on chat.chat_user = customer.customer_id left join departments on departments.admin_id = chat.admin_id  and departments.dept_id=chat.department where chat.admin_id = '$user_id' order by chat.chat_id DESC";
}else{
	...
	//$chat_detail_qry = "select chat.chat_id, chat.chat_user, chat.chat_type, chat.chat_status, chat.rating_value, customer.customer_name,customer.city,customer.country,customer.created_ip, TIME_FORMAT(chat.created_dt, '%H:%i') as chat_time, date_format(chat.created_dt, '%d/%m/%Y') as chat_dt, departments.department_name ,user.user_name,substring(user.agent_name,1,instr(user.agent_name,' ')-1) as agent_name from chat inner join chat_msg on chat_msg.chat_id=chat.chat_id left join customer on chat.chat_user = customer.customer_id left join user on chat_msg.msg_user_id = user.user_id left join departments on departments.admin_id = chat.admin_id  and departments.dept_id=chat.department where chat.admin_id = '$admin_id' and chat.department IN ($dept_id) order by chat.chat_id DESC";
	
	// user.user_name,substring(user.agent_name,1,instr(user.agent_name,' ')-1) as agent_name,user.chat_aviator,user.profile_image from chat inner join chat_msg on chat_msg.chat_id=chat.chat_id left join customer on chat.chat_user = customer.customer_id left join user on chat_msg.msg_user_id = user.user_id left join departments on chat.department = departments.dept_id where chat.chat_id = '$chat_id' order by chat_msg.chat_msg_id asc";
	

	$chat_detail_qry = "select chat.chat_id, chat.chat_user, chat.chat_type, chat.chat_status, chat.rating_value, (SELECT agent_name from user where user_id = (select MAX(msg_user_id) from chat_msg WHERE chat_id = chat.chat_id and msg_user_type=2)) as chat_agent, customer.customer_name,customer.city,customer.country,customer.created_ip, TIME_FORMAT(chat.created_dt, '%H:%i') as chat_time, date_format(chat.created_dt, '%d/%m/%Y') as chat_dt, departments.department_name from chat left join customer on chat.chat_user = customer.customer_id left join departments on departments.admin_id = chat.admin_id  and departments.dept_id=chat.department where chat.admin_id = '$admin_id' and chat.department IN ($dept_id) order by chat.chat_id DESC";
	...
	//echo "INSERT INTO chatbot_user(email,phone_no,admin_id) values ('$email','$ph_no','$admin_id')";exit;
	 ...
	 function send_notification($data){
		extract($data);
			...
			//$token='dx6toVgc-L1hhNykb4XPe_:APA91bEgRxqjx18fbIfp2jKesvt42M24JuhIGW8hbEhH3zeKgXM6XBhRayj1DMNUc4V2WM7BKveVSmuUztsT4heDAYCAq8ofKpDcNTwcyIScLiAZ2Adg53QbLpggKCdcAdnGxwLUdwKx';
