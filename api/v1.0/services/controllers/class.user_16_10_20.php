		...
		public function userLoginValidation($company_name, $user_name, $password){
            ...
            elseif($two_factor == 0){              
              if($result == null){
				...
				$this->db_query("UPDATE user SET login_status=1 WHERE user_id='$user_id'", array());
              }
              return $result;
            }
            else{              
...
public function createUser($agent_data){
    ...
    { 
      ...
      $qry_result = $this->db_query("INSERT INTO user (agent_name,phone_number,email_id,user_status,created_ip,created_by,incoming_call_status,user_type,user_name,user_pwd,sip_login,sip_username,sip_password,has_contact,has_sms,has_chat,has_whatsapp,has_chatbot,has_fb,has_wechat,has_telegram,has_internal_ticket,has_external_ticket,voice_3cx,predective_dialer,lead,wallboard_one,wallboard_two,wallboard_three,wallboard_four,layout,theme,timezone_id,admin_id,password,dsk_access,dsk_user_name,dsk_user_pwd,reports,close_all_menu,predective_dialer_behave,company_name,domain_name,hardware_id,has_fax,whatsapp_type,wp_instance_count,ext_int_status) VALUES ('$agent_name','$phone_number','$email_id',$user_status,'',$admin_id,1,4,'$user_name','$user_pwd','$sip_login','$sip_username','$sip_password',$has_contact,$has_sms,$has_chat,$has_whatsapp,$has_chatbot,$has_fb,$has_wechat,$has_telegram,$has_internal_ticket,$has_external_ticket,$voice_3cx,$predective_dialer,$lead,$wallboard_one,$wallboard_two,$wallboard_three,$wallboard_four,'$layout','$theme',$timezone_id,$admin_id,'$password','$dsk_access','$dsk_user_name','$dsk_user_pwd','$reports','$close_all_menu','$predective_dialer_behave','$company_name','$domain_name','$hardware_id','$has_fax','$whatsapp_type','$wp_instance_count',$ext_int_status)", array());    
      $result = $qry_result == 1 ? 1 : 0;
      if($qry_result != 0){
          $last_id_qry = "SELECT user_id FROM user ORDER BY user_id DESC LIMIT 1";
          $last_id = $this->fetchOne($last_id_qry,array());
...
public function admin_global_settings($data, $where_arr){   
 ...
 $qry_result = $this->db_query("UPDATE user SET logo_image='$logo_image_upload_path',small_logo_image='$small_logo_image_upload_path',ext_int_status='$ext_int_status' where admin_id='$user_id'", array());
    ...
    $resultqry = $qry_result == 1 ? 1 : 0; 
     if($resultqry==1){
        ...
        if($user_type==2){
          $admin_qry_result = $this->db_query("UPDATE admin_details SET ext_int_status='$ext_int_status',show_caller_id='$show_caller_id' where admin_id='$user_id'", array());
        }
        $result = array("status" => "true");          
        $tarray = json_encode($result);  
        print_r($tarray);exit;
        ...
        public function getAlluser($data){
            ...
            $search_qry = "";
            if($search_text != ""){
                $search_qry = " and (user.user_name like '%".$search_text."%' or user.email_id like '%".$search_text."%' or user.phone_number like '%".$search_text."%' or status.status_desc like '%".$search_text."%')";
            }
if($admin_id == '1'){
	  $qry = "select user.user_id,user.sip_login,user.user_name,user.user_pwd,user.agent_name,user.user_type, user_type.user_type_name, user.email_id,user.phone_number,user.user_status,user.has_contact,user.has_sms,user.has_chat,user.has_whatsapp,user.has_chatbot,user.has_fb,user.has_wechat,user.has_telegram,user.has_internal_ticket,user.has_external_ticket,user.voice_3cx,user.predective_dialer,user.lead,user.wallboard_one,user.wallboard_two,user.wallboard_three,user.wallboard_four,user.layout,user.theme,user.password,user.two_factor,user.company_name,user.domain_name,user.dsk_access,user.dsk_user_name,user.dsk_user_pwd,user.has_wallboard,user.sip_registered_status,user.hardware_id,user.has_fax,user.has_external_contact,user.reports,user.close_all_menu,user.has_internal_chat,user.login_status,user.external_contact_url,user.show_caller_id, status.status_name,status.status_desc from user left join user_type on user_type.user_type_id =user.user_type left join status on status.status_id = user.user_status where user.user_type != 1 ".$search_qry; 
	  ...
	  $qry = "select user.user_id,user.sip_login,user.user_name,user.user_pwd,user.agent_name,user.user_type, user_type.user_type_name, user.email_id,user.phone_number,user.user_status,user.has_contact,user.has_sms,user.has_chat,user.has_whatsapp,user.has_chatbot,user.has_fb,user.has_wechat,user.has_telegram,user.has_internal_ticket,user.has_external_ticket,user.voice_3cx,user.predective_dialer,user.lead,user.wallboard_one,user.wallboard_two,user.wallboard_three,user.wallboard_four,user.layout,user.theme,user.password,user.two_factor,user.company_name,user.domain_name,user.dsk_access,user.dsk_user_name,user.dsk_user_pwd,user.has_wallboard,user.sip_registered_status,user.hardware_id,user.has_fax,user.has_external_contact,user.reports,user.close_all_menu,user.login_status,user.external_contact_url,user.has_internal_chat,user.external_contact_url,user.show_caller_id, status.status_name,status.status_desc from user left join user_type on user_type.user_type_id =user.user_type left join status on status.status_id = user.user_status where user.user_type != 1 and user.delete_status != 1 and user.admin_id='$admin_id'".$search_qry; 
 ...
 /* if($has_contact == 1 || $has_contact == true){ $has_contacts = 1; }else{ $has_contacts = 0; }  
  ...
  //if($has_wallboard == 1 || $has_wallboard == true){ $has_wallboards = 1; }else{ $has_wallboards = 0; } */
	//echo "UPDATE user SET has_contact='$has_contact',has_sms='$has_sms',has_chat='$has_chat',user_status='$admin_status',has_whatsapp='$has_whatsapp',has_chatbot='$has_chatbot',has_fb='$has_fb',has_wechat='$has_wechat',has_telegram='$has_telegram',has_internal_ticket='$has_internal_ticket',has_external_ticket='$has_external_ticket',voice_3cx='$voice_3cx',predective_dialer='$predective_dialer',lead='$lead',wallboard_one='$wallboard_one',wallboard_two='$wallboard_two',wallboard_three='$wallboard_three',wallboard_four='$wallboard_four',two_factor='$two_factor',company_name='$company_name',domain_name='$domain_name',has_fax='$has_fax',reports='$reports',has_internal_chat='$has_internal_chat',whatsapp_num='$whatsapp_num' WHERE user_id='$admin_id'";exit;
		...
		$qry_result = $this->db_query("UPDATE whatsapp_instance_details SET admin_id='0' where admin_id = $admin_id", array());
   ...
   if($qry_result == 1){
    ...
    if($has_internal_chat == 1 || $has_internal_chat == true){ $has_internal_chats = 1; }else{ $has_internal_chats = 0; }
      //if($has_wallboard == 1 || $has_wallboard == true){ $has_wallboards = 1; }else{ $has_wallboards = 0; }
        ...
        $qry = "INSERT INTO admin_details(name,admin_id,pbx_count,agent_counts,has_contact,has_sms,has_chat,admin_status,has_whatsapp,has_chatbot,has_fb,has_wechat,has_telegram,has_internal_ticket,has_external_ticket,voice_3cx,predective_dialer,lead,wallboard_one,wallboard_two,wallboard_three,wallboard_four,two_factor,company_name,domain_name,has_fax,has_internal_chat,reports,price_sms,survey_vid,tarrif_id,whatsapp_type,wp_instance_count,reseller,sms_type,mr_voip,notes) VALUES ('$name', '$result','1','$agent_counts','$has_contacts','$has_smss','$has_chats',1,'$has_whatsapps','$has_chatbots','$has_fbs','$has_wechats','$has_telegrams','$has_internal_tickets','$has_external_tickets','$voice_3cxs','$predective_dialers','$leads','$wallboard_ones','$wallboard_twos','$wallboard_threes','$wallboard_fours','$two_factors','$company_name','$domain_name','$has_faxs','$has_internal_chats','$reports','$price_sms','$survey_vid','$tarrif_id','$whatsapp_type','$wp_instance_count','$reseller','$sms_type','$mr_voip','$notes')";
   $qry_result = $this->db_query($qry, array()); 
          $result = $qry_result == 1 ? 1 : 0;
...
public function change_agent_permission($data){
	...
	public function get_admin_global_settings($data){
		...
		$status = array('status' => 'true');
		$user_dialer_options = array('dialer_option' => $dialer, 'dialer_value' => $ext_int_status, 'show_caller_id'=> $show_caller_id);
...
public  function whatsapp_account_settings($data){  
	...
	public function dialer_settings($data){
      extract($data);//print_r($data);exit;
      $adminqry = "SELECT admin_id,user_id,sip_username,sip_password FROM user WHERE sip_login ='$ext_number' and company_name='$company_name'";
      $adminresult = $this->fetchData($adminqry,array());
      $adminresult_count = $this->dataRowCount($adminqry,array());
      if($adminresult_count > 0){
        $adminid = $adminresult['admin_id'];      
        if($adminid=='1'){
          $aid = $adminresult['user_id'];			
          $getqry = "SELECT sip_authentication,sip_password,sip_url,sip_port FROM pbx_settings WHERE sip_login ='$ext_number'";
          $qryresult = $this->fetchData($getqry,array());//print_r($qryresult);exit;
          $sipusername = base64_encode($qryresult['sip_authentication']);
          $sippassword = base64_encode($qryresult['sip_password']);
          $sipurl = base64_encode($qryresult['sip_url']);
          $sipport = base64_encode($qryresult['sip_port']);
          $result = array("sipusername"=>$sipusername,"sippassword"=>$sippassword,"sipurl"=>$sipurl,"sipport"=>$sipport);
          $status = array("status"=>"true");
          $sip_details_array = array("data"=>$result);	
          ...
          $aid = $adminid;			
          $getqry = "SELECT sip_authentication,sip_password,sip_url,sip_port FROM pbx_settings WHERE sip_login ='$ext_number'";
          $qryresult = $this->fetchData($getqry,array());
          $sipusername = base64_encode($qryresult['sip_authentication']);
          $sippassword = base64_encode($qryresult['sip_password']);
          $sipurl = base64_encode($qryresult['sip_url']);
          $sipport = base64_encode($qryresult['sip_port']);
...
if($count > 0){
    ...
    {
      $email_url='https://omni.mconnectapps.com/';
    }
   // echo $email_url;exit;
