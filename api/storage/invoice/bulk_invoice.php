...
for($i=0; $i<(count($export_data)); $i++){
	//echo $export_data[$i]['invoice_no'];
	//exit;

 $invoice_no=$export_data[$i]['invoice_no'];
 $created_at=$export_data[$i]['created_at'];
 $date = new DateTime($created_at);
 $iv_dt = $date->format('d-m-Y');
$due=date('Y-m-d', strtotime($iv_dt. ' + 15 days'));

  $admin_id=$export_data[$i]['admin_id'];
  $user_id=$export_data[$i]['user_id'];
  $company_name=$export_data[$i]['company_name'];
  $biller_city=$export_data[$i]['biller_city'];
  $biller_add1=$export_data[$i]['biller_add1'];
  $biller_add2=$export_data[$i]['biller_add2'];
  $biller_state=$export_data[$i]['biller_state'];
  $biller_country=$export_data[$i]['biller_country'];
  $biller_zip_code=$export_data[$i]['biller_zip_code'];
  $biller_phone=$export_data[$i]['biller_phone'];
  $biller_toll_free=$export_data[$i]['biller_toll_free'];
  $biller_logo_image=$export_data[$i]['biller_logo_image'];
  ...
  $biller_email=$export_data[$i]['biller_email'];
  $biller_reg_no=$export_data[$i]['biller_reg_no'];
  $prevmonth=$export_data[$i]['prevmonth'];
  $biller_account_no=$export_data[$i]['biller_account_no'];
  $biller_bank='Bank :  '.$export_data[$i]['biller_bank'];
  $branch='Branch :  '.$export_data[$i]['biller_branch'];
  $prevmonth = date("d M Y", strtotime($prevmonth));  
  $prevEnd=$export_data[$i]['prevEnd'];
 $prevEnd = date("d M Y", strtotime($prevEnd));  
$period= $prevmonth.' to '.$prevEnd;

  $agent_name=$export_data[$i]['agent_name'];
  $ship_to=$export_data[$i]['ship_to'];
  $ship_add1=$export_data[$i]['ship_add1'];
  $ship_add2=$export_data[$i]['ship_add2'];
  $ship_city=$export_data[$i]['ship_city'];
  $ship_state=$export_data[$i]['ship_state'];
  $ship_country=$export_data[$i]['ship_country'];
  $ship_zip=$export_data[$i]['ship_zip'];
  $monthly_charges=$export_data[$i]['monthly_charges'];
  $amt=$export_data[$i]['amt'];
	if($amt==''){
		$amt='0';
	}
  $btax=$monthly_charges+$amt;
	//echo $btax;exit;
  $discount=$export_data[$i]['discount'];
  $tax_name=$export_data[$i]['tax_name'];
$dis_rate=$btax-round((($discount / 100) * $btax),2);
  $tax_per=round($export_data[$i]['tax_per'],2);
if($tax_name!=''){
